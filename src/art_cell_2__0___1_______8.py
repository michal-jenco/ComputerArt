...
def main():
    ...
    interesting_functions = {0: lambda x, y: ((sin(sin(x + i * sin(y - i ** (.0087 * i))))) + 1) * 2,
                             ...
                             14: lambda x, y: sqrt(abs(sin(cos(y + i / 10.))) * 8)
                                              * sqrt(abs(sin(cos(x - i / 30.))) * 6) % 4,
                             15: lambda x, y: abs(i / 10. + sin(x) + sin(x / 3.) + sin(x / 5.)
                                                  + cos(y / 2.) + cos(y / 7.)) % 4,
                             16: lambda x, y: (gcd(x, y) + i) % 4,
                             17: lambda x, y: log(x + y + i + 1, 2) % 4,
                             18: lambda x, y: abs(sin(x*y+i))}

    interesting_functions2 = {0: lambda x, y: 1}

    for i in range(100):
        print("Animation: %s" % i)
        prob_dist = ProbabilityDistribution(value_func=interesting_functions[0])
